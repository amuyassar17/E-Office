<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
                                ...
                                echo '<script language="javascript">window.history.back();</script>';
                                    ...
                                    if(!preg_match("/^[0-9.-]*$/", $tgl_surat)){
                                        ...
                                        if(!preg_match("/^[a-zA-Z0-9.,()\/ -]*$/", $jenis_surat)){
                                            $_SESSION['jenis_suratk'] = 'Form jenis_surat hanya boleh mengandung karakter huruf, angka, spasi, titik(.), koma(,), minus(-), garis miring(/), dan kurung()';
                                            echo '<script language="javascript">window.history.back();</script>';
                                        } else {
                                            ...
                                            if($file != ""){
                                                ...
                                                if(in_array($eks, $ekstensi) == true){
                                                    if($ukuran < 2500000){
                                                        ...
                                                        if(!empty($file)){
                                                            ...
                                                            $query = mysqli_query($config, "UPDATE tbl_surat_keluar SET no_agenda='$no_agenda',tujuan='$tujuan',no_surat='$no_surat',isi='$isi',tgl_surat='$tgl_surat',file='$nfile',jenis_surat='$jenis_surat',id_user='$id_user' WHERE id_surat='$id_surat'");
                                                ...
                                                $query = mysqli_query($config, "UPDATE tbl_surat_keluar SET no_agenda='$no_agenda',tujuan='$tujuan',no_surat='$no_surat',isi='$isi',tgl_surat='$tgl_surat',jenis_surat='$jenis_surat',id_user='$id_user' WHERE id_surat='$id_surat'");
            ...
            $query = mysqli_query($config, "SELECT id_surat, no_agenda, tujuan, no_surat, isi, tgl_surat, file, jenis_surat, id_user FROM tbl_surat_keluar WHERE id_surat='$id_surat'");
            list($id_surat, $no_agenda, $tujuan, $no_surat, $isi, $tgl_surat, $file, $jenis_surat, $id_user) = mysqli_fetch_array($query);
